/* ==========================================================
   ...
   ========================================================== */
    ...
    /* ==========================================================
       ...
       ========================================================== */
    function rollFish() {
      const bait = BAITS[currentBaitKey];
      const rod = RODS[player.rodId] || RODS.bamboo;
      const rand = Math.random() * 100;
      let selectedRarity = "Common";

      // Бонус к редкой рыбе от удочки
      const bonus = rod.rareBonus || 0;
      const rareWeights = {
        Common: Math.max(10, bait.rarityWeights.Common - bonus),
        Rare: bait.rarityWeights.Rare + (bonus * 0.5),
        Epic: bait.rarityWeights.Epic + (bonus * 0.35),
        Legendary: bait.rarityWeights.Legendary + (bonus * 0.15)
      };

      let accum = 0;
      for (const [rarity, chance] of Object.entries(rareWeights)) {
        accum += chance;
        if (rand <= accum) {
      ...
      const pool = FISH_DATABASE.filter(f => f.rarity === selectedRarity);
      const chosen = pool.length ? pool[Math.floor(Math.random() * pool.length)] : FISH_DATABASE[0];

      // Случайный вес с нормальным распределением к середине
      const r = (Math.random() + Math.random()) / 2;
      const weight = +(chosen.min_weight + r * (chosen.max_weight - chosen.min_weight)).toFixed(2);

      return { fish: chosen, weight };
    }
    ...
    function updateReeling(dt) {
      ...
      // Интенсивность рывков зависит от редкости рыбы
      const rarityMultiplier = {
        Common: 1.0,
        Rare: 1.25,
        Epic: 1.5,
        Legendary: 1.8
      }[activeFish.rarity] || 1.0;
      ...
      if (fishFightChangeTimer <= 0) {
        ...
        fishFightChangeTimer = 0.8 + Math.random() * 1.5;
      }
      ...
      // Рыба плавно тянет и подергивает
      const fishSine = Math.sin(Date.now() * 0.004) * 8 * rarityMultiplier;
      tension += (fishFightDir * 14 * rarityMultiplier * dt) + (fishSine * dt);
...
// Фаза поклёвки
    ...
    function render() {
      ...
      if (gameState === "CASTING") {
        ...
        if (p >= 1) {
          ...
          // Таймер поклёвки
          const bait = BAITS[currentBaitKey];
          const waitTime = (bait.waitMin + Math.random() * (bait.waitMax - bait.waitMin)) / bait.speedMultiplier;
    ...
    // Процедурная отрисовка рыбы под водой
    function drawUnderwaterFish(x, y, angle, tailOsc, colorHex) {
      ...
      ctx.fillStyle = "#000";
      ctx.beginPath();
      ctx.arc(14.5, -3, 1.2, 0, Math.PI * 2);
      ctx.fill();

      // Хвост с синусоидальным изгибом
      const tailX = -20;
      const tailY = Math.sin(tailOsc) * 8;
      ctx.beginPath();
      ctx.moveTo(-16, 0);
