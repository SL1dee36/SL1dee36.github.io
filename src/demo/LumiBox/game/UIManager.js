...
export class UIManager {
    constructor(inputManager, settingsManager, saveAndQuitCallback) {
    ...
    bindHTMLElements() {
        ...
        this.worldsMenuScreen = document.getElementById('worlds-menu-screen');
        this.createWorldScreen = document.getElementById('create-world-screen');
    ...
    closeAllMenus() {
        ...
        if (this.worldsMenuScreen) this.worldsMenuScreen.style.display = 'none';
        if (this.createWorldScreen) this.createWorldScreen.style.display = 'none';
        if (this.pauseMenu) this.pauseMenu.style.display = 'none';
        if (this.optionsMenu) this.optionsMenu.style.display = 'none';
        if (this.statsMenu) this.statsMenu.style.display = 'none';
    }

    updateHeartsHUD(hp, maxHp = 20) {
        if (!this.heartsContainer) return;
        const playerController = this.inventoryComponent?.gameObject?.getComponent('PlayerController');
        if (playerController && playerController.gameMode === 'creative') {
        ...
        const totalHearts = Math.floor(maxHp / 2);
        for (let i = 0; i < totalHearts; i++) {
            const heartVal = hp - (i * 2);
            let state = 'empty';
            if (heartVal >= 2) state = 'full';
            else if (heartVal === 1) state = 'half';

            const heart = document.createElement('div');
            heart.className = 'heart-icon';
            heart.innerHTML = this.getHeartSvg(state);
    ...
    getHeartSvg(state) {
        if (state === 'full') {
            return `<svg viewBox="0 0 9 9" width="18" height="18"><path fill="#000" d="M2,0 h2 v1 h1 v-1 h2 v1 h1 v2 h-1 v1 h-1 v1 h-1 v1 h-1 v1 h-1 v-1 h-1 v-1 h-1 v-1 h-1 v-2 h1 z"/><path fill="#f00" d="M2,1 h2 v1 h1 v-1 h2 v2 h-1 v1 h-1 v1 h-1 v1 h-1 v-1 h-1 v-1 h-1 v-2 h1 z"/><path fill="#fff" d="M2,2 h1 v1 h-1 z"/></svg>`;
        } else if (state === 'half') {
            return `<svg viewBox="0 0 9 9" width="18" height="18"><path fill="#000" d="M2,0 h2 v1 h1 v-1 h2 v1 h1 v2 h-1 v1 h-1 v1 h-1 v1 h-1 v1 h-1 v-1 h-1 v-1 h-1 v-1 h-1 v-2 h1 z"/><path fill="#444" d="M5,1 h2 v2 h-1 v1 h-1 v1 h-1 v1 h1 v-1 h1 v-1 h1 v-2 h-2 z"/><path fill="#f00" d="M2,1 h2 v5 h-1 v-1 h-1 v-1 h-1 v-2 h1 z"/><path fill="#fff" d="M2,2 h1 v1 h-1 z"/></svg>`;
        } else {
            return `<svg viewBox="0 0 9 9" width="18" height="18"><path fill="#000" d="M2,0 h2 v1 h1 v-1 h2 v1 h1 v2 h-1 v1 h-1 v1 h-1 v1 h-1 v1 h-1 v-1 h-1 v-1 h-1 v-1 h-1 v-2 h1 z"/><path fill="#444" d="M2,1 h2 v1 h1 v-1 h2 v2 h-1 v1 h-1 v1 h-1 v1 h-1 v-1 h-1 v-1 h-1 v-2 h1 z"/></svg>`;
        }
    }
    ...
    initKeyboardShortcuts() {
        document.addEventListener('keydown', (e) => {
            if (!this.inputManager.isUIOpen || !this.hoveredSlot) return;
            if (e.code.startsWith('Digit')) {
                const digit = parseInt(e.code.replace('Digit', ''), 10);
                if (digit >= 1 && digit <= 9) {
                    this.handleHoverKeySwap(this.hoveredSlot.type, this.hoveredSlot.index, digit - 1);
                }
            }
        });
    }

    initOutsideClickDrop() {
        document.addEventListener('mousedown', (e) => {
            if (!this.inputManager.isUIOpen || !this.cursorItem) return;
            const clickedInsideSlot = e.target.closest('.slot');
            const clickedInsideButton = e.target.closest('button') || e.target.closest('input');

            if (!clickedInsideSlot && !clickedInsideButton) {
                if (e.target === this.inventoryElement || !e.target.closest('#inventory')) {
                    if (e.button === 2) {
                        this.dropItemIntoWorld({ id: this.cursorItem.id, count: 1 });
                        this.cursorItem.count--;
                        if (this.cursorItem.count <= 0) this.cursorItem = null;
                    } else if (e.button === 0) {
                        this.dropItemIntoWorld(this.cursorItem);
                        this.cursorItem = null;
                    }
                    this.updateAll();
                }
    ...
    dropItemIntoWorld(item) {
        if (!item || !this.inventoryComponent) return;
        const player = this.inventoryComponent.gameObject;
        const world = player.engine.physicsEngine.world;
        const camera = player.engine.renderer.camera;
        ...
        spawnPos.addScaledVector(lookDir, 0.5);

        const dropVel = lookDir.clone().multiplyScalar(5.5);
        dropVel.y += 1.8;

        world.spawnItemDrop(spawnPos.x, spawnPos.y, spawnPos.z, item.id, item.count, dropVel);
    }

    handleHoverKeySwap(sourceType, sourceIndex, hotbarIndex) {
        const sourceList = this.getListByType(sourceType);
        const hotbarList = this.inventoryComponent.hotbar;
        if (!sourceList || !hotbarList) return;

        if (sourceType === 'hotbar' && sourceIndex === hotbarIndex) return;

        const temp = sourceList[sourceIndex];
        sourceList[sourceIndex] = hotbarList[hotbarIndex];
        hotbarList[hotbarIndex] = temp;

        this.recalcRecipes(sourceType);
        this.updateAll();
    }
    ...
    initPauseMenu() {
        document.getElementById('resume-btn').onclick = () => {
            ...
            setTimeout(() => this.inputManager.lock(), 10);
        };
    ...
    updateStatsUI() {
        ...
        if (el) el.innerHTML = "LuminaCraft v1.4<br>Engine: LuminaEngine (Nazaryan A.K.)";
    ...
    createSlotElement(item, i, type, clickHandler) {
        ...
        s.onmouseenter = () => {
            if (type !== 'hotbar_hud') this.hoveredSlot = { type, index: i };
        };
        s.onmouseleave = () => {
            if (this.hoveredSlot && this.hoveredSlot.type === type && this.hoveredSlot.index === i) {
                this.hoveredSlot = null;
            }
        };
        ...
        s.onmousedown = (e) => {
            ...
            e.preventDefault();
            e.stopPropagation();
            if (clickHandler) clickHandler(i, e.button === 2, e.shiftKey);
            if (this.cursorItem) {
    ...
    handleMobileTap(type, index) {
        if (this.cursorItem) {
            this.handleSlotClick(type, index, false, false);
            this.selectedSlotIndex = -1;
            return;
        }
    ...
    handleMobileLongPress(type, index) {
        this.handleSlotClick(type, index, true, false);
        this.selectedSlotIndex = -1;
        this.selectedSlotType = null;
    }
    ...
    updateAll() {
        ...
        const playerController = this.inventoryComponent.gameObject.getComponent('PlayerController');
        if (playerController) {
            this.updateHeartsHUD(playerController.hp, playerController.maxHp);
        }
        if (this.inputManager.isUIOpen) {
            this.updateInventoryWindow();
            this.updateCursorIcon();
    ...
    updateInventoryWindow() {
        ...
        if (this.inventoryComponent.activeContainerType === 'workbench') {
            ...
            for (let i = 0; i < 9; i++) {
                gridDiv.appendChild(this.createSlotElement(this.inventoryComponent.containerGrid[i], i, 'container', (idx, rb, sh) => this.handleSlotClick('container', idx, rb, sh)));
            }
            ...
            containerDiv.appendChild(this.createSlotElement(this.inventoryComponent.containerResult, 0, 'result', (idx, rb, sh) => this.handleCraftResultClick(true, sh)));
        } else if (this.inventoryComponent.activeContainerType === 'furnace') {
            const leftCol = document.createElement('div');
            leftCol.appendChild(this.createSlotElement(this.inventoryComponent.containerGrid[0], 0, 'container', (idx, rb, sh) => this.handleSlotClick('container', idx, rb, sh)));
            ...
            leftCol.appendChild(this.createSlotElement(this.inventoryComponent.containerGrid[1], 1, 'container', (idx, rb, sh) => this.handleSlotClick('container', idx, rb, sh)));
            ...
            containerDiv.appendChild(arrow);
            containerDiv.appendChild(this.createSlotElement(this.inventoryComponent.containerGrid[2], 2, 'result', (idx, rb, sh) => this.handleSlotClick('container', idx, rb, sh)));
        } else {
            const gridDiv = document.createElement('div');
            gridDiv.style.display = 'grid';
            ...
            gridDiv.className = 'inventory-grid';
            for (let i = 0; i < 4; i++) {
                gridDiv.appendChild(this.createSlotElement(this.inventoryComponent.crafting2x2[i], i, 'crafting', (idx, rb, sh) => this.handleSlotClick('crafting', idx, rb, sh)));
            }
            containerDiv.appendChild(gridDiv);
            const arrow = document.createElement('div'); arrow.innerHTML = '&#10132;'; arrow.style.fontSize = '20px'; arrow.style.margin = '0 10px';
            containerDiv.appendChild(arrow);
            containerDiv.appendChild(this.createSlotElement(this.inventoryComponent.craftingResult, 0, 'result', (idx, rb, sh) => this.handleCraftResultClick(false, sh)));
        }
        ...
        for (let i = 0; i < 27; i++) {
            mainGrid.appendChild(this.createSlotElement(this.inventoryComponent.main[i], i, 'main', (idx, rb, sh) => this.handleSlotClick('main', idx, rb, sh)));
        }
        ...
        for (let i = 0; i < 9; i++) {
            hotbarGrid.appendChild(this.createSlotElement(this.inventoryComponent.hotbar[i], i, 'hotbar', (idx, rb, sh) => this.handleSlotClick('hotbar', idx, rb, sh)));
        }
    ...
    handleSlotClick(type, index, isRightClick, isShiftClick = false) {
        const list = this.getListByType(type);
        if (!list) return;
        const clickedItem = list[index];

        if (isShiftClick && clickedItem) {
            this.handleQuickMove(type, index);
    ...
    handleQuickMove(sourceType, index) {
        const sourceList = this.getListByType(sourceType);
        ...
        if (sourceType === 'hotbar') {
            remaining = this.inventoryComponent._addToStacks(this.inventoryComponent.main, item.id, remaining);
            if (remaining > 0) remaining = this.inventoryComponent._addToEmpty(this.inventoryComponent.main, item.id, remaining);
        } else if (sourceType === 'main') {
            remaining = this.inventoryComponent._addToStacks(this.inventoryComponent.hotbar, item.id, remaining);
            if (remaining > 0) remaining = this.inventoryComponent._addToEmpty(this.inventoryComponent.hotbar, item.id, remaining);
        } else if (sourceType === 'crafting' || sourceType === 'container') {
            remaining = this.inventoryComponent._addToStacks(this.inventoryComponent.hotbar, item.id, remaining);
            if (remaining > 0) remaining = this.inventoryComponent._addToStacks(this.inventoryComponent.main, item.id, remaining);
            if (remaining > 0) remaining = this.inventoryComponent._addToEmpty(this.inventoryComponent.hotbar, item.id, remaining);
            if (remaining > 0) remaining = this.inventoryComponent._addToEmpty(this.inventoryComponent.main, item.id, remaining);
        }
    ...
    handleCraftResultClick(isWorkbench, isShift = false) {
        const inv = this.inventoryComponent;
        let resultItem = isWorkbench ? inv.containerResult : inv.craftingResult;
        const grid = isWorkbench ? inv.containerGrid : inv.crafting2x2;
        if (!resultItem) return;

        if (isShift) {
            while (resultItem) {
                const added = inv.addItem(resultItem.id, resultItem.count);
                if (!added) break;
                this.consumeIngredients(grid);
                this.updateCraftingResult();
                resultItem = isWorkbench ? inv.containerResult : inv.craftingResult;
            }
        } else {
            if (!this.cursorItem) {
